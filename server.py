...
Provides REST endpoints for:
  • Single + batch + campaign call dispatch (LiveKit agent_dispatch)
  • CRUD on appointments, call logs, agent profiles, campaigns, contacts
  • Settings management (BYOK) backed by Supabase
  • SIP trunk creation + log streaming + stats

Also runs APScheduler to fire scheduled campaigns.
"""
...
# ── Patch SSL with certifi BEFORE LiveKit/Supabase import ────────────────────
...
from db import (  # noqa: E402
    APP_STATE_KEYS, CREDENTIAL_KEYS, SENSITIVE_KEYS,
    cancel_appointment, clear_errors, create_campaign, create_agent_profile,
    ...
    get_calls_by_phone, get_campaign, get_contacts, get_logs, get_setting, get_stats,
    init_db, log_error, save_settings, set_default_agent_profile, set_setting,
    update_agent_profile, update_call_notes, update_campaign_run_stats,
    update_campaign_status,
)
from prompts import DEFAULT_SYSTEM_PROMPT  # noqa: E402
...
def eff(key: str) -> str:
    """Effective value of a credential — read ONLY from os.environ.

    The VPS environment is the single source of truth for all credentials.
    Supabase is never consulted for these keys.
    """
...
OPTIONAL_CRED_KEYS = [
    ...
    "GEMINI_MODEL", "GEMINI_TTS_VOICE", "USE_GEMINI_REALTIME",
    "DEEPGRAM_API_KEY",
...
async def api_env_check():
    """Deployment verification: shows which env vars are set on this VPS.

    All values are masked except sentinel-style ones (URLs, model names,
    region). Useful as a sanity check after deploying to Coolify/Docker.
    """
    def _status(k: str) -> dict:
        v = os.getenv(k, "")
        if not v:
        ...
        return {"configured": True, "value": v}
    required = {k: _status(k) for k in REQUIRED_CRED_KEYS}
    optional = {k: _status(k) for k in OPTIONAL_CRED_KEYS}
    missing_required = [k for k, v in required.items() if not v["configured"]]
    return {
        "deployment_ready": len(missing_required) == 0,
        "missing_required": missing_required,
        "required": required,
        "optional": optional,
        "source": "VPS environment variables",
    }
...
async def api_save_settings(req: SettingsRequest):
    """Persist app-state settings only.

    Credential keys are silently rejected — the VPS environment is the only
    source of truth for those. The response lists what was rejected so the
    ...
    return {
        ...
        "message": (
            f"Cannot edit credentials from the dashboard — set them as VPS env vars: "
...
async def api_setup_trunk():
    """Create the SIP outbound trunk in LiveKit.

    The new trunk_id is RETURNED to the caller — it is NOT persisted to
    Supabase or written into ``os.environ``. The user must add
    ``OUTBOUND_TRUNK_ID=<id>`` to the VPS environment variables and redeploy.
    """
    url = eff("LIVEKIT_URL")
    key = eff("LIVEKIT_API_KEY")
    secret = eff("LIVEKIT_API_SECRET")
    ...
    missing = [k for k, v in [
        ...
        ("VOBIZ_SIP_DOMAIN", sip_domain), ("VOBIZ_USERNAME", username),
        ("VOBIZ_PASSWORD", password), ("VOBIZ_OUTBOUND_NUMBER", phone),
    ] if not v]
    if missing:
        raise HTTPException(400, f"Missing VPS env vars: {', '.join(missing)}")
    ...
    try:
        ...
        return {
            ...
            "trunk_id": trunk_id,
            "action_required": (
                f"Add OUTBOUND_TRUNK_ID={trunk_id} to your VPS environment "
                f"variables and redeploy. The agent will not pick up the new "
                f"trunk until it sees it in os.environ."
            ),
        }
    except Exception as exc:
...
async def _dispatch_one(
    lk, lk_api_module, contact: dict, room_name: str,
...
async def _run_campaign(campaign_id: str) -> None:
    ...
    if not (url and key and secret):
        logger.error("Campaign %s: LiveKit env vars not configured on the VPS", campaign_id)
        return
    ...
    try:
        ...
        for i, contact in enumerate(contacts):
            ...
            if not phone.startswith("+"):
                ...
                continue
            room_name = (
                f"camp-{campaign_id[:8]}-{phone.replace('+','')}-{random.randint(100,999)}"
            )
...
def _schedule_campaign(campaign_id: str, schedule_type: str, schedule_time: str) -> None:
    ...
    _scheduler.add_job(
        _run_campaign, trigger=trigger, args=[campaign_id], id=job_id, replace_existing=True
    )
...
async def api_update_campaign_status(campaign_id: str, req: StatusRequest):
    ...
    if req.status == "paused" and _scheduler and _scheduler.get_job(job_id):
        ...
        if campaign and campaign.get("schedule_type") in ("daily", "weekdays"):
            _schedule_campaign(
                campaign_id, campaign["schedule_type"], campaign.get("schedule_time", "09:00")
            )
